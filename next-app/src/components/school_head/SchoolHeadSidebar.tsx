"use client";
import { ChevronRight, ClipboardList, FolderClock, LayoutDashboard, Shield, X } from "lucide-react";

const NAV_ITEMS = [
  { key: "dashboard", label: "Dashboard", icon: LayoutDashboard },
  { key: "requests", label: "Requests", icon: ClipboardList },
  { key: "history", label: "Records", icon: FolderClock },
];

interface SchoolHeadSidebarProps {
  currentPage: string;
...
export default function SchoolHeadSidebar({ currentPage, setCurrentPage, sidebarOpen, setSidebarOpen, currentUser }: SchoolHeadSidebarProps) {
  const NavBlock = (
    <ul className="space-y-1">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <li key={item.key}>
            <button type="button" onClick={() => { setCurrentPage(item.key); setSidebarOpen(false); }} className={`flex w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left text-sm transition-all duration-150 ${isActive ? "bg-emerald-500/15 font-semibold text-white shadow-sm ring-1 ring-emerald-400/25" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}>
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col bg-navy-900 px-4 py-5 shadow-[4px_0_24px_rgba(0,0,0,0.08)] lg:flex" style={bgStyle}>
        ...
        <div className="mt-auto border-t border-slate-800 pt-4">
          <button type="button" className="flex w-full items-center gap-3 rounded-2xl px-2 py-2 text-left transition-colors hover:bg-white/5">
            ...
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-white">{currentUser?.full_name || "School Head"}</p><p className="truncate text-xs text-slate-500">{currentUser?.email || "head@gmail.com"}</p></div>
