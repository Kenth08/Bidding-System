    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      ...
      <div className={`relative flex max-h-[calc(100vh-2rem)] w-full flex-col ${SIZE_CLASS[size] || SIZE_CLASS.lg} overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-2xl`}>
        <div className="flex shrink-0 items-start justify-between border-b border-slate-100 px-6 py-5">
          ...
          <button type="button" onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100" aria-label="Close modal">
            <X className="h-4 w-4" />
          </button>
        </div>
