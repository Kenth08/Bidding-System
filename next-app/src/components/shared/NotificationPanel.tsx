...
export default function NotificationPanel({ onNavigate }: { onNavigate?: (link: string, item: unknown) => void }) {
  ...
  function resolveNavigationTarget(item: NotificationItem) {
    ...
    if (!link) return { link: "", projectId: null as string | null };
    try {
      const parsed = new URL(link, window.location.origin);
      return { link: parsed.pathname, projectId: parsed.searchParams.get("project") };
    } catch {
      const [path, query = ""] = link.split("?");
      return { link: path, projectId: new URLSearchParams(query).get("project") };
    }
  }
      ...
      {open ? (
        <div className="absolute right-0 top-12 z-50 w-80 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
            ...
            {unreadCount > 0 ? (
              <button type="button" onClick={handleMarkAllRead} className="text-xs font-medium text-emerald-500 transition-colors hover:text-emerald-600">Mark all as read</button>
            ) : null}
          </div>

          <div className="max-h-72 divide-y divide-slate-50 overflow-y-auto">
            {loading ? (
              <div className="flex items-center justify-center py-8"><div className="h-5 w-5 animate-spin rounded-full border-2 border-emerald-500 border-t-transparent" /></div>
            ) : notifications.length === 0 ? (
              <div className="px-4 py-10 text-center">
                <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100"><Bell className="h-5 w-5 text-slate-300" /></div>
              ...
              notifications.map((item) => {
                ...
                return (
                  <button key={item.id} type="button" onClick={() => handleNotificationClick(item)} className={`flex w-full items-start gap-3 px-4 py-3 text-left transition-colors ${item.is_read ? "hover:bg-slate-50" : "border-l-2 border-l-emerald-400 bg-emerald-50/30 hover:bg-emerald-50/50"}`}>
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${config.bg}`}><IconComp className={`h-4 w-4 ${config.color}`} /></div>
