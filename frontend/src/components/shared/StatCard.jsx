 ...
 * }} props
 */
export default function StatCard({ title, value, icon: Icon, subtitle = "Updated just now", accentLine = false, leftBorderColor = null, iconColor = null }) {
  const ResolvedIcon = Icon || BarChart3;

  const containerStyle = leftBorderColor ? { borderLeft: `4px solid ${leftBorderColor}` } : undefined;
  const iconStyle = iconColor ? { color: iconColor } : undefined;

  return (
    <article
      style={containerStyle}
      className={`relative overflow-hidden rounded-[20px] border bg-white/95 p-5 shadow-[0_10px_30px_rgba(15,23,42,0.04)] backdrop-blur transition-all duration-150 hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(15,23,42,0.08)] ${accentLine ? "border-emerald-100" : "border-slate-100 hover:border-slate-200"}`}
    >
